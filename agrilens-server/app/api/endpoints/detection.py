...
async def detect_disease(
    ...
    # --- Guardrail Check: Validate if the image contains a rice plant ---
    if settings.openai_api_key:
        guardrail_enabled = True
        try:
            from app.models.setting import AppSetting
            from sqlalchemy import select
            q = select(AppSetting).where(AppSetting.key == "openai_guardrail")
            res = await db.execute(q)
            setting_entry = res.scalar_one_or_none()
            if setting_entry and setting_entry.value == "false":
                guardrail_enabled = False
        except Exception as db_exc:
            logger.error("Failed to query OpenAI guardrail setting: %s", str(db_exc))

        if guardrail_enabled:
            try:
                explainer = request.app.state.explainer
                llm_engine = explainer._engine
                guardrail = llm_engine.validate_rice_plant_image(content)
                if not guardrail.get("is_rice_plant", True):
                    reason = guardrail.get("reason", "Gambar tidak terdeteksi sebagai tanaman padi.")
                    logger.warning("Guardrail blocked image | reason=%s", reason)
                    return JSONResponse(
                        status_code=400,
                        content={"detail": f"Format atau subjek gambar tidak sesuai: {reason}"},
                    )
            except Exception as exc:
                logger.error("Guardrail check failed (ignored to fail-open): %s", str(exc))
