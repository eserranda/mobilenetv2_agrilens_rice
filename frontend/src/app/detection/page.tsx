...
function DetectionContent() {
  ...
  // --- Reset hook to clear screen from layout trigger ---
  useEffect(() => {
    if (resetParam === "true") {
  ...
  useEffect(() => {
    if (historyId) {
      ...
      // Clear screen state if navigating back to root and no local file is currently selected
      if (!selectedFile && !resetParam) {
        setSelectedFile(null);
        setPreviewUrl(null);
        setResult(null);
        setError(null);
  ...
  }, [historyId, selectedFile, resetParam]);
